...
type Features struct {
	FeatureId    int64     `orm:"auto" orm:"omitempty"`
...
// Id doesn't exist
func GetFeaturesById(id int64) (v *Features, err error) {
	o := orm.NewOrm()
	v = &Features{FeatureId: id}
	if err = o.QueryTable(new(Features)).Filter("FeatureId", id).RelatedSel().One(v); err == nil {
		return v, nil
	}
	return nil, err
}
...
func UpdateFeaturesById(m *Features) (err error) {
	o := orm.NewOrm()
	v := Features{FeatureId: m.FeatureId}
...
func DeleteFeatures(id int64) (err error) {
	o := orm.NewOrm()
	v := Features{FeatureId: id}
	// ascertain id exists in the database
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&Features{FeatureId: id}); err == nil {
