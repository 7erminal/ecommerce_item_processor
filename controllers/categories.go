...
func (c *CategoriesController) Post() {
	...
	logs.Info("Data received is ", c.Ctx.Input.Query("CategoryName"))
...
func (c *CategoriesController) GetOne() {
	...
	if err != nil {
		logs.Error("An error occurred while fetching category ", err.Error())
...
func (c *CategoriesController) Put() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.ParseInt(idStr, 0, 64)
	v := models.Categories{CategoryId: id}
	json.Unmarshal(c.Ctx.Input.RequestBody, &v)
